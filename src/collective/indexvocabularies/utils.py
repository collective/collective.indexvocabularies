...
def _create_querystring_registry(index):
    ...
    registry.records[f'{PREFIX}.{index}.title'] = Record(field.TextLine(title="Title"), index)
    registry.records[f'{PREFIX}.{index}.description'] = Record(field.Text(title="Description"), "")
    registry.records[f'{PREFIX}.{index}.fetch_vocabulary'] = Record(field.Bool(title="Fetch vocabulary"), True)
    registry.records[f'{PREFIX}.{index}.enabled'] = Record(field.Bool(title="Enabled"), True)
    registry.records[f'{PREFIX}.{index}.group'] = Record(field.Text(title="Group"), "Metadata")
    registry.records[f'{PREFIX}.{index}.operations'] = Record(field.List(title="Operations", value_type=field.TextLine()), ['plone.app.querystring.operation.selection.any', 'plone.app.querystring.operation.selection.all', 'plone.app.querystring.operation.selection.none'])
    registry.records[f'{PREFIX}.{index}.sortable'] = Record(field.Bool(title="Sortable"), False)
